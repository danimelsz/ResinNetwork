...
##############################################
### 2.1 Singlelayer: All MBSDs (Dataset 1) ###
...
singlelayer = singlelayer[c(0:68), ] # keep only rows of bees
singlelayer = singlelayer[, 69:ncol(singlelayer)] # keep only columns of plants
singlelayer <- empty(singlelayer, count = F) # remove nodes with no interactions
# NODF
observado_NODF_g = networklevel(singlelayer, index = "NODF") # empirical NODF = 34.44
observado_NODF_g
# Modules
modules = computeModules(singlelayer, method = "Beckett")
modules@likelihood # M = 0.4080618
...
write.table(modules.dataframe$Rows_modules, "data/modulemembership.rows.plants_Dataset1.txt", sep="\t", quote=F) # export rows (plants)
write.table(modules.dataframe$Cols_modules, "data/modulemembership.cols.bees_Dataset1.txt", sep="\t", quote=F) # export columns (bees)
...
######################################
### 2.5 SINGLELAYER: CHEMICAL + FIELDWORK (Dataset 2) ###
...
nodes_cf = read.delim("data/net1nodes_Dataset2.txt", header = T) # node traits
links_cf = read.delim("data/list.Dataset2.txt", header = T) # list of interactions
...
singlelayer_cf = singlelayer_cf[c(0:61), ] # keep only rows of bees
singlelayer_cf = singlelayer_cf[, 62:ncol(singlelayer_cf)] # keep only columns of plants
singlelayer_cf = empty(singlelayer_cf, count = F) # remove nodes with no interactions
dim(singlelayer_cf)
# NODF
observado_NODF_cf = networklevel(singlelayer_cf, index = "NODF") # empirical NODF = 18.56588
observado_NODF_cf
# Modules
...
write.table(modules.dataframe$Rows_modules, "data/modulemembership.rows.plants_Dataset2.txt", sep="\t", quote=F) # export rows (plants)
write.table(modules.dataframe$Cols_modules, "data/modulemembership.cols.bees_Dataset2.txt", sep="\t", quote=F) # export columns (bees)

##################################
### 2.5 SINGLELAYER: DATASET 3 ###

# Load edge and node lists
nodes_3 = read.delim("data/net1nodes_Dataset3.txt", header = T) # node traits
links_3 = read.delim("data/list.Dataset3.txt", header = T) # list of interactions
# Inspect objects
head(nodes_3)
class(nodes_3)
head(links_3)
class(links_3)
unique(links_3$layer)
# Load network
singlelayer_3 = graph_from_data_frame(d=links_3, vertices=nodes_3, directed=F)
# Inspect network
class(singlelayer_3)
singlelayer_3
V(singlelayer_3)$name
V(singlelayer_3)$taxon
# Create an adjacent matrix for bipartite
singlelayer_3 = get.adjacency(singlelayer_3,sparse=FALSE)
singlelayer_3 = singlelayer_3[c(0:60), ] # keep only rows of bees
singlelayer_3 = singlelayer_3[, 61:ncol(singlelayer_3)] # keep only columns of plants
singlelayer_3 = empty(singlelayer_3, count = F) # remove nodes with no interactions
dim(singlelayer_3)
# NODF
observado_NODF_3 = networklevel(singlelayer_3, index = "NODF") # empirical NODF = 8.44 
observado_NODF_3
# Modules
modules_3 = computeModules(singlelayer_3, method = "Beckett")
modules_3@likelihood # M = 0.63
# Compute module membership
modules.list = listModuleInformation(modules_3) # create empty list of modules
source("R/Extracting_modules_from_bipartite.R") # fuction for extracting modules
modules.dataframe = modules_from_bipartite(modules.list)
write.table(modules.dataframe$Rows_modules, "data/modulemembership.rows.bees_Dataset3.txt", sep="\t", quote=F) # export rows (plants)
write.table(modules.dataframe$Cols_modules, "data/modulemembership.cols.plants_Dataset3.txt", sep="\t", quote=F) # export columns (bees)
...
#############################################
### 3.1 MULTILAYER: ALL MBSDs (DATASET 1) ###
...
modules=read.table("data/partitions_Dataset1.txt", h=T)
head(modules)
sort(unique(modules$module)) # check the number of modules (= 6)
...
# Set NODE colors for modules (= 6 modules)
...
# Plot
png(filename= "figures/multilayer_Dataset1_labels.png", res= 300, height= 3000, width= 4900)
...
plot(net1_multi, 
     ...
     vertex.label.cex=.2, # .02
     edge.color = E(net1_multi)$color, 
     edge.curved=0.3, 
     layout=l)
title(main = "Total multilayer network (Dataset 1)", cex.main=2)
...
#################################
### 3.5 MULTILAYER: CHEMICAL + FIELDWORK (DATASET 2) ###
...
# Import module membership
modules=read.table("data/partitions_Dataset2.txt", h=T)
head(modules)
sort(unique(modules$module)) # check the number of modules (= 8)
...
vertex_name_order = data.frame(nodes=V(net1_multi)$name)
modules_order = merge(vertex_name_order, modules, by="nodes", sort = F)
#colrs = rainbow(8, alpha=1) #rainbow palette with 8 colors
colrs = c("#1F78B4", "#B2DF8A", "#FB9A99", "#FF7F00", "#CAB2D6", "#FFFF99", "#AB2737", "#CCA737") #colorblind-friendly palette with 12 colors.
V(net1_multi)$color <- colrs[modules_order$module]
# Plot
png(filename= "figures/multilayer_Dataset2.png", res= 300, height= 3000, width= 4900)
par(mfrow=c(1,1),mar=c(1,1,3,17))
plot(net1_multi, 
     vertex.color = V(net1_multi)$color, 
     vertex.frame.color= V(net1_multi)$color, 
...
#################################
### 3.5 MULTILAYER: DATASET 3 ###

# Create network
net1_multi = graph_from_data_frame(d=links_3, vertices=nodes_3, directed=F) 
...
V(net1_multi)$taxon # plant or bee
V(net1_multi)$type # 1 = bee, 2 = plant
E(net1_multi)$layer # Neotropics or Indo-Malaya, Australia or Afrotropics
E(net1_multi)$weight # 1 for all

# Import module membership
modules=read.table("data/partitions_Dataset3.txt", h=T)
head(modules)
sort(unique(modules$module)) # check the number of modules (= 10)
...
#V(net1_multi)$color = gsub("plant","#2C8437",V(net1_multi)$color)
# Set EDGE colors for the layers (biogeographic regions)
unique(net1_multi$layer) # check the number of modules (= 11)
E(net1_multi)$color = E(net1_multi)$layer
E(net1_multi)$color = gsub("Neotropics","#B3DCF9",E(net1_multi)$color)
E(net1_multi)$color = gsub("Indo-Malaya","#ded0b2",E(net1_multi)$color)
E(net1_multi)$color = gsub("Australia","#ADD1A2",E(net1_multi)$color)
E(net1_multi)$color = gsub("Afrotropics","#A151A1",E(net1_multi)$color)
# Set polarity
E(net1_multi)$arrow.mode = 0
# Set EDGE width
E(net1_multi)$width = E(net1_multi)$layer
E(net1_multi)$width = gsub("Neotropics", 1, E(net1_multi)$width)
E(net1_multi)$width = gsub("Indo-Malaya", 1, E(net1_multi)$width)
E(net1_multi)$width = gsub("Australia", 1, E(net1_multi)$width)
E(net1_multi)$width = gsub("Afrotropics", 1, E(net1_multi)$width)
# Set NODE colors for modules (= 10 modules)
vertex_name_order = data.frame(nodes=V(net1_multi)$name)
modules_order = merge(vertex_name_order, modules, by="nodes", sort = F)
#colrs = rainbow(10, alpha=1) #rainbow palette with 10 colors
colrs = c("#1F78B4", "#B2DF8A", "#FB9A99", "#FF7F00", "#CAB2D6", "#FFFF99", "#AB2737", "#CCA737", "#1F2737", "#AAAAAA") #colorblind-friendly palette with 11 colors.
V(net1_multi)$color <- colrs[modules_order$module]
# Plot
png(filename= "figures/multilayer_Dataset3_labels.png", res= 300, height= 3000, width= 4900)
par(mfrow=c(1,1),mar=c(1,1,3,17))
plot(net1_multi, 
     vertex.color = V(net1_multi)$color, 
     vertex.frame.color= V(net1_multi)$color, 
     ...
     edge.curved=0.3, 
     layout=l)
title(main = "Plant-bee resin foraging multilayer network (Dataset 3)", cex.main=2)
legend(x = 1.3,y = 0.9, title="Taxon",
       legend = c("Bees", "Plants"), pch = c(15,16),   
       text.col = "gray20", title.col = "black", 
       box.lwd = 0, cex = 1.5, col=c("grey", "grey"))
legend(x = 1.3,y = 0.1, title="Layers (biogeographic regions)",
       legend = c("Afrotropics", "Indo-Malaya, Australia", "Neotropics"),
       fill = c("#A151A1", "#ded0b2", "#ADD1A2", "#B3DCF9"), border = "white", 
       text.col = "gray20", title.col = "black", box.lwd = 0, cex = 1.5)
legend(x = 1.3,y = -0.6, title="Modules",
       legend = c("node colors = modules"),
       fill = c("grey"), border = "white", 
